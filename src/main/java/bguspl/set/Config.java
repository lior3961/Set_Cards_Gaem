...
public class Config {
        ...
        if (filename == null || filename.isEmpty())
            ...
            try (InputStream is = Files.newInputStream(Paths.get(filename))) {
                ...
                try (InputStream is = Config.class.getClassLoader().getResourceAsStream(filename)) {
                    ...
                    logger.severe(
                            "warning: cannot read config file from the resources directory either. Using defaults.");
                }
            }
    ...
    public Config(Logger logger, Properties properties) {
        ...
        turnTimeoutMillis = (long) (Double.parseDouble(properties.getProperty("TurnTimeoutSeconds", "60")) * 300.0);
        ...
        for (int i = 0; i < players; i++) {
            ...
            if (playerKeysString.length() > 0) {
                ...
                if (codes.length != tableSize)
                    logger.severe("warning: player " + (i + 1) + " keys (" + codes.length + ") mismatch table size ("
                            + tableSize + ").");
